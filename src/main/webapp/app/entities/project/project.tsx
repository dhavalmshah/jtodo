            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('icon')}>
                  Icon <FontAwesomeIcon icon={getSortIconByFieldName('icon')} />
                </th>
                <th>
            ...
            <tbody>
              {projectList.map((project, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{project.color}</td>
                  <td>{project.icon}</td>
                  <td>{project.owner ? <Link to={`/user-attributes/${project.owner.id}`}>{project.owner.id}</Link> : ''}</td>
                  <td>
                    {project.members
                      ? project.members.map((val, j) => (
                          <span key={j}>
                            <Link to={`/user-attributes/${val.id}`}>{val.id}</Link>
                            {j === project.members.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
